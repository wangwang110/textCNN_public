...
import pdb
from model import *
from utils_v2 import prepare_data_for_cnn, prepare_data_for_rnn, \
...
def run_model(opt, train, val, test, wordtoix, ixtoword):
    ...
    with tf.Session(config=config) as sess:
        ...
        newx=cPickle.load(open('./data/test_data','rb'))
        ...
        for epoch in range(opt.max_epochs):
            ...
            for _, train_index in ks:
                ...
                ## 随便选组模拟数据
                index = np.random.choice(len(train), 2*opt.batch_size)
                ## 为了后面剔除掉一些
